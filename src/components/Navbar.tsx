  ...
  return (
    <>
      ...
      <motion.nav
        ...
        className={`fixed left-1/2 -translate-x-1/2 bottom-[max(0.6rem,env(safe-area-inset-bottom))] md:bottom-7 w-[94vw] md:w-[min(1100px,94vw)] z-50 border-2 rounded-full backdrop-blur-lg flex justify-between md:justify-center items-center px-4 py-1.5 md:px-8 md:py-3 shadow-[0_0_50px_rgba(139,92,246,0.25)] gap-3 md:gap-14 whitespace-nowrap transition-all duration-300 ${scrolled ? 'border-prisma-purple/50 bg-prisma-dark/98 scale-[0.98]' : 'border-white/10 bg-prisma-dark/75'}`}
      >
        <button
          onClick={() => scrollTo('events')}
          className="text-lg leading-none sm:text-2xl md:text-3xl uppercase hover:text-prisma-accent transition-all duration-300 font-display tracking-[0.02em] md:tracking-[0.1em] shrink text-white opacity-95"
        >
          Eventos
        </button>
        ...
        <button
          onClick={onLogoClick}
          className="logo-aura mx-2 md:mx-4 px-1 select-none flex items-center justify-center flex-shrink-0"
        >
          {logoUrl ? (
            <img src={logoUrl} alt={siteName} className="logo-aura-img h-12 sm:h-16 md:h-20 lg:h-22 w-auto object-contain transition-transform duration-500 hover:scale-110" />
          ) : (
            <>
              <span className="logo-aura-text font-display text-4xl sm:text-5xl md:text-7xl text-white tracking-tighter flex items-center gap-2 sm:hidden">
                {shortName}
              </span>
        ...
        <button
          ...
          className="relative w-8 h-5 sm:w-11 sm:h-7 md:w-14 md:h-8 z-50 flex-shrink-0 cursor-pointer"
          aria-label="Menu"
        >
          <motion.span
            animate={isOpen ? { rotate: 45, top: "50%", y: "-50%" } : { rotate: 0, top: "0%", y: "0%" }}
            className="absolute left-0 w-full h-[3px] bg-white block origin-center transition-all"
          />
          <motion.span
